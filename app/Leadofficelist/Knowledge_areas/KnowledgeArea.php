...
class KnowledgeArea extends \BaseModel {
	...
	public static function getKnowledgeAreasForFormSelect( $blank_entry = false, $blank_message = 'Please select...', $prefix = "" ) {
		...
		foreach(KnowledgeAreaGroup::all() as $group) {
			if ( $blank_entry ) {
				$areas[$group->id][''] = $blank_message;
			}
			foreach ( KnowledgeArea::where('knowledge_area_group_id', '=', $group->id)->orderBy( 'name', 'ASC' )->get( [ 'id', 'name' ] ) as $area ) {
				$areas[$group->id][ $area->id ] = $prefix . $area->name;
			}
		}
